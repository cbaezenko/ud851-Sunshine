...
//        int date = 1234234214;
//        String description = "a nice day";
//        double high = -1.5;
//        double low = -27.3;

        long dateInMillis = mCursor.getLong(MainActivity.DATE);
        String dateString = SunshineDateUtils.getFriendlyDateString(mContext,dateInMillis,false);

        int weather_condition = mCursor.getInt(MainActivity.DESCRIPTION);

        String description = SunshineWeatherUtils.getStringForWeatherCondition(mContext,weather_condition);

        String weatherForThisDay =dateString+" - "+
                description+" - "+
                mCursor.getDouble(MainActivity.TEMP_HIGH)+" - "+
                mCursor.getDouble(MainActivity.TEMP_LOW);
...
//  TODO (11) Create a new method that allows you to swap Cursors.
    public void swapCursor(Cursor cursor){
        mCursor=cursor;
      //  mContext.getContentResolver().notifyChange(cursor.getNotificationUri(),);
        notifyDataSetChanged();
    }
